...
class CliffWalkingEnv:
  ...
  def createP(self):
    # 初始化
    P = [[[] for j in range(4)] for i in range(self.nrow * self.ncol)]  # shape = (self.nrow * self.ncol, 4, 0)
...
class PolicyIteration:
  ...
  def policy_evaluation(self):  # 策略评估
    ...
    while 1:
      ...
      cnt += 1
    logging.info(f'策略评估进行{cnt}轮后完成')
      ...
      for a in env.P[s]:
          for s_ in env.P[s][a]:
              ...
              if s_[3] == True:
                  holes.add(s_[1])
  holes = holes - ends
  logging.info(f'冰洞的索引:{holes}')
  logging.info(f'目标的索引:{ends}')
